...
class Database:
    ...
    # ORDER operations
    def add_order(self, tg_id, name, pickup_type, location, payment_type, details, pickup_location, total_amount):
        sql = '''
        INSERT INTO orders 
            (tg_id, name, pickup_type, location,
            payment_type, details, pickup_location, total_amount)
        VALUES 
            (?, ?, ?, ?, ?, ?, ?, ?, ?)'''
        self.execute(
            sql,
    ...
    def update_order(self, status, order_id, tg_id, transtion_id=None):
        sql = '''UPDATE orders SET status = ?, transtion_id = ? WHERE id = ?
        WHERE tg_id = ? AND id = ?'''
        self.execute(sql, parameters=(status, transtion_id, tg_id, order_id),
                     commit=True)
